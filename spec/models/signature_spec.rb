...
RSpec.describe Signature, type: :model do
  ...
  describe "callbacks" do
    context "when the signature is destroyed" do
      let!(:petition) { FactoryBot.create(:open_petition, :translated) }
  ...
  context "validations" do
    ...
    it { is_expected.to allow_value("Please click this petitions.parliament.scot for a special offer").for(:name) }
    it { is_expected.not_to allow_value("Please click this https://petitions.parliament.scot for a special offer").for(:name) }
  ...
  describe "#number" do
    let!(:petition) { FactoryBot.create(:open_petition, :translated) }
    let!(:creator) { petition.creator }
    let!(:other_petition) { FactoryBot.create(:open_petition, :translated) }
    ...
    context "when the petition is open" do
      ...
      context "and the signer is not from the UK" do
        ...
        context "and the postcode is set" do
          ...
          it "doesn't search for the constituency and doesn't set constituency_id" do
